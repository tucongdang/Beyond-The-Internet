  ...
  const modalContent = (
    ...
    >
      ...
      >
        ...
        {/* Structured Header Row (No overlapping absolute positioning) */}
        <div className="w-full flex items-center justify-between pb-3 border-b border-white/10 mb-3 shrink-0">
          ...
          {/* Action controls on right: Fullscreen Toggle, Inactivity Timer & Close */}
          <div className="flex items-center gap-1.5 shrink-0">
            {/* Fullscreen projector view toggle */}
            <button
            ...
            >
              {displayMode === 'fullscreen' ? (
                <>
                  ...
                  <span className="text-[10px]">{localLanguage === 'en' ? 'Compact' : 'Thu Gọn'}</span>
                </>
              ) : (
                <>
                  <Maximize2 className="w-3.5 h-3.5 text-[#F7CAC9]" />
                  <span className="text-[10px]">{localLanguage === 'en' ? 'Fullscreen' : 'Phóng To'}</span>
                </>
              )}
            </button>
        ...
        {/* Quick Instructions */}
        <div className="w-full fluent-box-nested border border-white/10 rounded-[6px] p-2.5 sm:p-3 my-2 text-left text-xs space-y-1 shrink-0">
